...
public class UnivastApiService
{
    ...
    {
        var url = $"api/v1/navigation?campusId={Uri.EscapeDataString(campusId)}";
        ...
        CancellationToken ct = default)
    {
        ...
        using var response = await _http.PostAsJsonAsync("api/v1/routes/route", payload, JsonOptions, ct);
        await EnsureSuccessOrThrowAsync(response, ct);
        ...
        CancellationToken ct = default)
    {
        ...
        using var response = await _http.PostAsJsonAsync("api/v1/routes/nearest-node", payload, JsonOptions, ct);
        await EnsureSuccessOrThrowAsync(response, ct);
