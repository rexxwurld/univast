...
public partial class RoutePage : ContentPage
{
    ...
    // Set via the "campusId" query parameter (e.g. navigating here from a
    // Place that has a sourceCampusId — see PlaceDetailPage). Falls back to
    // ApiConfig.DefaultCampusId when arriving from MainPage's plain
    // "🧭 Navigate" button, which has no specific place/campus in mind.
    ...
    private async void UseMyLocation_Clicked(object? sender, EventArgs e)
    {
        ...
        try
        {
            ...
            // Make sure navigation nodes are available.
            if (_nodes.Count == 0)
            {
                _nodes = await _api.GetNavigationNodesAsync(_campusId);
            ...
            if (_nodes.Count == 0)
            {
                ShowStatus("This campus has no navigation nodes yet.");
                return;
            }
            ...
            // Ask the backend for the navigation node nearest to the phone.
            var nearest = await _api.GetNearestNodeAsync(
                _campusId,
                location.Latitude,
                location.Longitude);
            ...
            // Match the backend node to the node in our picker list.
            var matched = _nodes.FirstOrDefault(
                n => n.Id == nearest.Node.Id);

            if (matched is null)
            {
                ShowStatus("Nearest node was not found in the campus node list.");
                return;
            }
            ...
            ShowStatus(
                $"Starting point: {matched.DisplayName}");
        }
        catch (UnivastApiException ex)
        {
            ShowStatus(
                $"Couldn't find the nearest node: {ex.Message}");
        }
        catch (Exception ex)
        {
            ShowStatus(
                $"Couldn't get your location: {ex.Message}");
        }
        finally
        {
            SetBusy(false);
    ...
    private void DrawRoute(RouteResponse route)
    {
        ...
        for (var i = 0; i < route.Nodes.Count; i++)
        {
            ...
            var color = i == 0
                ? new MapsuiColor(46, 125, 50)   // start: green
                : i == route.Nodes.Count - 1
                    ? new MapsuiColor(198, 40, 40) // end: red
                    : new MapsuiColor(25, 118, 210); // waypoint: blue
        ...
        var routeFeature = new GeometryFeature(lineString)
        {
            Styles = new List<IStyle>
            {
                new VectorStyle
                {
                    Line = new Pen(new MapsuiColor(25, 118, 210), 5),
                },
            },
        };
